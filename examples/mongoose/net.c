...
static void handle_leds_get(struct mg_connection *c,
                            struct mg_http_message *hm) {
  ...
  mg_http_reply(c, 200, s_json_header, "%d\n", val);
}

static void handle_leds_toogle(struct mg_connection *c,
                               struct mg_http_message *hm) {
...
void net_init(struct mg_mgr *mgr) {
  s_settings.device_name = strdup("Ardusimple-SBC");
